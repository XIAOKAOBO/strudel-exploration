...
def combine_colfeature_vector(cell_feature_vector: pandas.DataFrame, line_feature_vector: pandas.DataFrame):
    cell_fv_df = line_feature_vector.drop(['label'], axis=1).merge(cell_feature_vector,
                                                                   left_on=['file_name', 'sheet_name', 'column_number'],
                                                                   right_on=['file_name', 'sheet_name', 'column_index'])
    cell_fv_df = cell_fv_df.drop(['column_number'], axis=1)
    return cell_fv_df
...
class CStrudel:
    ...
    def fit(self, train_set: pandas.DataFrame, test_set: pandas.DataFrame):
        ...
        result = pandas.concat([test_set_cell_profile, pred.fillna(0), y_test], axis=1)
