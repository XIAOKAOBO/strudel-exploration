...
def create_column_feature_vector(file_json_dict):
    ...
    line_number = np.array(range(len(file_array.columns)))
    ...
    def empty_line_ratio_after(window):
        row_window = window[0]
        rows_after = np.array(file_array.loc[:,row_window[1:]]).transpose()
        empty_rows_after = [row_after for row_after in rows_after if is_empty_row(row_after)]
    ...
    windows = list(rolling.Apply(file_array.transpose(), num_lookup_lines + 1, operation=list, window_type='variable'))
    windows = np.array([windows[len(windows) - len(file_array):]], dtype=object).transpose()
    ...
    def empty_column_ratio_before(window):
        row_window = window[0]
        rows_before = np.array(file_array.loc[:,row_window[:-1]]).transpose()
    ...
    def hist_diff_after(window):
        row_window = window[0]
        row = np.array(file_array.loc[:,row_window[0]])
    ...
    def data_type_before(window):
        row_window = window[0]
        row = np.array(file_array.loc[:,row_window[-1]])
        row_data_type = [detect_datatype(cell) for cell in row]
        rows_before = np.array(file_array.loc[:,row_window[:-1]]).transpose()
        data_type_same_ratio = 0.0
    ...
    for row, is_derived_row in zip(file_array_t, is_derived_file_array):
    ...
    # print(file_json_dict['file_name']+' '+file_json_dict['table_id'] +' '+str(line_number))
    # print(labels.shape)
    ...
    lstrudel_feature_vector = pd.DataFrame({'file_name': tname,
                                                'sheet_name': tid,
...
class COStrudel:
    ...
    def fit(self, train_set: pd.DataFrame, test_set: pd.DataFrame, method='predict_proba'):
        ...
        clean_train_set = empty_line_removed_train_set.drop(profile_columns, axis=0)
        clean_test_set = empty_line_removed_test_set.drop(profile_columns, axis=0)
        ...
        pred_prob_df = pd.DataFrame(data=pred_prob, columns=clf.classes_ + '_prob')

        test_set_with_line_prob = pd.concat([test_set_line_profile, pred_prob_df, y_test], axis=0)
