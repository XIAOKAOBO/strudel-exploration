...
from strudel.cstrudel import CStrudel, combine_colfeature_vector, create_cell_feature_vector, combine_feature_vector
...
from strudel.utility import process_pebble_results, cprocess_pebble_results
from strudel.costrudel import COStrudel, create_column_feature_vector
...
if __name__ == '__main__':
    ...
    if test_dataset is None:
        ...
        print(dataset.shape)
        print('Creating colstrudel feature vector...')
        with ProcessPool(max_workers=max_workers, max_tasks=max_tasks) as pool:
            optional_column_feature_vectors = pool.map(create_column_feature_vector, dataset).result()

        col_fvs_dataset = process_pebble_results(optional_column_feature_vectors)
        # print(len(optional_column_feature_vcectors))
        # co =0
        # for item in optional_column_feature_vectors:
        #     # print(item.shape)
        #     co+=1
        #     print(co)
        print('LOL')

        print('Cross-validating colstrudel...')
        cv = CrossValidation(n_splits=10)
        col_cv_results = cv.cross_validate(col_fvs_dataset, COStrudel.algo_name)     
        ...
        cell_fvs_dataset = combine_colfeature_vector(cell_fvs_dataset,col_cv_results)
        for item in cell_fvs_dataset:
            print(cell_fvs_dataset[item])
